...
!
! adapted from PHONON/PH/delta_sphi.f90  (minor modification)
!-----------------------------------------------------------------------------------
SUBROUTINE ph_delta_sphi (npw, npwq, na, icart, nah, ihubst, wfcatomk_, wfcatomkpq_,   & 
  ...
  !
  use dvhub_mod, only: ofsbeta
  !  
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN) :: npw, npwq, na, icart, nah, ihubst 
  ! index of k point
  ! index of k+q point
  ...
  IF (iflag == 1) THEN
     ...
     IF ( okvan ) THEN
        ...
        ENDDO
        !
        !jjz: we drop support for bgrp in qe2pert
