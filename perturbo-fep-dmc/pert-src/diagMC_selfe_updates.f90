...
module selfE_update_multiband   
    
   use pert_param, only : LatticeFrolich, gauss_frolich
   ...
   subroutine add_ph(diagram, stat)
      ...
      real(dp) :: Pq, Pnu(Nph),Ptau12, Pib, Pvtex, temp  !P(B)
      real(dp) :: P_A,P_B,D_AB, P_accept
      real(dp) :: decay, decay_exp
      real(dp) :: ran, err_add,sigma_q
      ...
      sigma_q = sqrt(1.d0/tau12)
      call sample_q_frolich_omp(diagram%seed, sigma_q, vn1%q, Pq, 1)
      ...
      !prob for choose
      Pvtex = 1.d0 / diagram%order; 
      Pib = 1.d0 / nbnd
      !P_B = Pq * Pel_line * Pib * Pnu(nu) * Ptau12 * config%PA(1) 
      P_B = Pq * Pvtex * Ptau12 * config%PA(1)         
      !prob ratio W(B)/W(A) 
      ...
      !if(LatticeFrolich) D_AB = D_AB / (twopi**3) 
   ...
   subroutine remove_ph(diagram, stat)
      ...
      real(dp) :: xqt(3)
      real(dp) :: Pq,Pnu(Nph), Pib, Pvtex          !P(B)
      ...
      sigma_q = sqrt(1.d0/tau12)
      call sample_q_frolich_omp(diagram%seed, sigma_q, xqt, Pq)
      Pvtex = 1.d0 / (diagram%order-2) !there are order-2 vertex of the lower order diagram 
      P_A = Ptau12 * Pq * Pvtex * config%PA(1)
   ...
   subroutine update_vertex_time(diagram, stat)
      ...
      real(dp) :: sign_wq, wq,ran, Paccept 
   ...
   subroutine update_external(diagram, stat)
      ...
      integer :: ib 
      real(dp) :: P_band(nbnd),wq,ek, oldT, P_accept, ran, tau_ph  
      complex(dp) :: phase 
   ...
   subroutine update_swap(diagram, stat)
      ...
      if(Equal(vRn%kout,config%P)) return !check whether there are reducible diagram 
   ...
   !from 1st order self-energy to gt = 1
   subroutine ToFake(diagram, stat)
      ...
      tauLR = vR%tau 
      if(vL%tau>1e-10) stop 'head time not equal 0'
      ![1.1] P_B 
      Pq = 1.d0 
      do nu = 1,Nph 
      ...
      ![1.2] D_AB
      D_AB = FakeRatio / ( abs(vL%gkq(vL%n_out,vL%n_in,vL%nu))**2 * Dph( vL%wq(vL%nu),tauLR )  )
      D_AB =  D_AB /( Gel(vL%ekout(vL%n_out),tauLR) )
      P_accept = D_AB * P_B 
      ...
      if(ran < P_accept) then 
         ...
         if(.not.Equal(vL%kin,config%P)) stop 'moemntum error'
      endif 
       

   end subroutine

   !from gt = 1 to first order self-energy 
...
module selfE_update  
   ...
   subroutine update_vertex_time(diagram, stat)
      ...
      wq = vA%wq(vA%nu)
      decay = vA%ekin(vA%n_in) - vA%ekout(vA%n_out) - wq
      if(vA%tau>tauQ)  decay = decay + 2.d0*wq
      !if(vA%link(1).eq.1) decay = decay - vA%ekin(vA%n_in)
      !if(vA%link(3).eq.maxN) decay = decay + vA%ekout(vA%n_out)
      call exp_sample_omp(diagram%seed,tauL,tauR,decay,tau1,Pt,1)

      Paccept = Dph( wq, abs(tau1-tauQ) ) / Dph( wq, abs(vA%tau-tauQ) ) !ph gf change 
      Paccept = Paccept / exp( -wq*abs(tau1-vA%tau) ) 
      call random_number_omp(diagram%seed,ran)
      if(ran < Paccept) then 
         vA%tau = tau1
         stat%accept(5) = stat%accept(5) + 1
   ...
   subroutine update_tau(diagram, stat)
      ...
      decay = vR%ekin(vR%n_in)
      if(diagram%order.ne.0) decay = decay + vR%wq(vR%nu) 
   ...
   subroutine init_selfe_diagram(diagram)
      ...
      vi%link = (/ maxN, -1, 2 /)
      call cal_ek(vi%kout,vi%ekout,vi%ukout); vi%ukin = vi%ukout 
      vi%n_in = config%ib; vi%n_out = config%ib
