...
from input_parameters import print_inputs_to_html
...
html.write('Click on a calculation mode to get its requirements, outputs, and input file.\n\n')
html.write('<br>\n'*2)
...
for node in interactive_nodes:
   ...
   if node == 'qe2pert':
      tutorial_link = ' <a href="mydoc_qe2pert.html#calc_mode_{}">See in tutorial.</a>'.format(node)
   else :
      tutorial_link = ' <a href="mydoc_perturbo.html#calc_mode_{}">See in tutorial.</a>'.format(node)
...
for file_req in files_desc_dict.keys():
   ...
   if 'format example' in files_desc_dict[file_req].keys():
      html.write('<a href="{}" target="_blank">File format example.</a>'.format(files_desc_dict[file_req]['format example']))
   html.write('    </div>\n')
